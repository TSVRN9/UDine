...
Infer the repo from `git remote -v` — `gh` does this automatically when run inside a clone. Note: no GitHub remote is configured for this repo yet, so these commands won't work until it's pushed to a GitHub repo.
