...
/** Finds a hall's Grab 'N Go entry among get_infov2's retail locations (the strip's open/closed
 * state comes from the shared hours client, same feed the café/market rows already use). Matches
 * by "starts with the hall name" + a loose, case-insensitive "grab" substring rather than the exact
 * live title -- the feed uses a smart apostrophe ("Worcester Grab ‘N Go", U+2018, not a straight
 * "'"), which a literal string match would silently miss. */
 ...
 * `/grab-n-go/[slug]` route is retired -- Grab 'N Go is now the hall-menu screen's 4th tab, reusing
 * the same station-grouped rendering, plate, and log pipeline instead of a forked screen. */
