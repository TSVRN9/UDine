...
describe("paneIndexForSwipe", () => {
  ...
  describe("velocity-based fling commit", () => {
    ...
    // velocity is in the tens-to-low-hundreds, not a sub-1 fraction. On-device measurement
    // (emulator-5556, a slow deliberate drag released gently) read ~0-280pts/s. 100 sits in that
    // band -- a real slow drag with SOME residual velocity should still snap back, not commit.
    // This would have incorrectly committed under the old (pre-fix) 0.5 threshold, since any
    // realistic points/second velocity clears 0.5.
    ...
    // realistic points/second magnitude (on-device: ~830-3300pts/s for an unambiguous flick).
