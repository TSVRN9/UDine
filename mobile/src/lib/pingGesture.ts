...
export type PingRow = { sender_id: string; receiver_id: string; hall_tid: number; message: string };

/** Maps a payload + the sender's id onto the `pings` table's exact column names -- pins the
 * contract #93 specifies ({ message, hall_tid }) as a plain, red-green-testable function
 * independent of the Supabase client wiring that actually calls it. */
export function pingRow(senderId: string, payload: PingPayload): PingRow {
  return { sender_id: senderId, receiver_id: payload.friendId, hall_tid: payload.hallTid, message: payload.message };
}
